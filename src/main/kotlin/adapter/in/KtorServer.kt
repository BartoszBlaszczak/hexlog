...
object KtorServer {
	private fun server() = embeddedServer(Netty, configure = {
		if (AppProperties.useSSL) connectSSL()
		else connector { port = AppProperties.httpPort }
	}) {
		routing {
		...
		install(StatusPages) {
			...
			exception<Throwable> { call, exception ->
				exception.localizedMessage?.let(logger::error)
	...
	private fun ApplicationEngine.Configuration.connectSSL() {
